...
Wraps `ctx.subagents.start` / `ctx.subagents.startContinuable` — the exact seam both the `subagent` tool and `run_code`'s `tools.subagent` call — so every spawn without an explicit model choice (neither `agentOptions` nor `modelSelection`) is intercepted and the human is asked which model + reasoning effort to use.
...
| `parentModelRecommendation` | `boolean` | `true` | When `true`, one tier is tagged **(Recommended)** and moved first. The tag comes from a **deterministic classifier** (task length + complexity signals — `review`/`audit`/`harden`/`debug`/`refactor`/`migrat`/`implement`/`architect`/`design`/`security`/`performance`/`concurr`/`integrat`/`complex`/`thread`/`async`/`schema`/`migration`/`contract`): hard signals or >900 chars → Correctness, <200 chars without signals → Cost, else Balanced. Zero cost, instant, never flakes. When `false`, the default order is used and no tag is computed. |
| `askWhenExplicit` | `boolean` | `false` | When `false`, spawns that already carry an explicit model choice — in **either** `agentOptions` or the canonical `modelSelection` field (`provider`/`model`/`reasoningEffort` only — sizing keys like `maxTokens` do not count) — are not intercepted. When `true`, they are asked too. |
...
| `qualityRank` | `Record<string, number>` | `{}` | Optional quality ranking — `"provider/model": number`. The **Correctness** tier exists ONLY when at least one entry is ranked (price is never a quality signal); it picks the top-ranked entry at its deepest declared effort. Unranked entries never win. |
| `recommender` | `{ provider, model, reasoningEffort? } \| undefined` | `undefined` | Optional route that opts into an **LLM judgment** for the recommendation tag (the deterministic classifier is the default). When set, that route classifies the task at its **lowest declared effort** (or `reasoningEffort` when given) and the reply must be exactly `simple`/`medium`/`hard` — an invalid reply throws and no tier is tagged (fail-open, never a fabricated recommendation). |
...
- **Honest tier semantics.** `Balanced` is the configured default model at its configured effort; `Correctness` is the top `qualityRank` entry at its deepest declared effort — and is **omitted entirely** when no entry is ranked (price is never a quality signal); `Cost` is the lowest estimated price at its lowest effort. Each tier's description states its criterion (e.g. "Top quality rank (N) — …" vs "Lowest estimated price — …" vs "Configured default — …") and renders `model default` when the tier has no effort of its own. Prices are blended per-1M estimates (` (est. $X.XX/1M)`) and shown only when priced; unknown-price models are never tier picks.
...
- **Three tiers + manual list.** A deterministic classifier (task length + complexity signals; LLM judgment only when `recommender` is configured) tags one tier `(Recommended)` with a visible rationale (`looks hard`/`simple`/`medium` complexity) and moves it first. The manual list adds up to `maxManualOptions` cheapest remaining models (excluding the three tier models via `skipEntries` dedupe). Labels are decided **before** registration via `uniqueChoiceLabel` (displayed label can never decode to nothing): base → `base (provider)` → `base (provider/model)` collision ladder. `priceSuffix` renders ` (est. $0.00/1M)` for zero-cost priced entries (cost 0 is real, not unknown).
- **Free-text + follow-up.** Free text resolves deterministically first (`deterministicMatch` + `effortFromText` — declared ids matched at identifier boundaries + superlative synonyms incl. `xtra`/`deep` → deepest declared, `min` → lowest); only ambiguous text goes to a bounded LLM shortlist (≤12, `boundedTable`). `llmText` consumes only `text-delta` (a `block-end` carries the same fully assembled block — appending both would double the text), handles terminal `finish` chunks (`reason.kind: error|aborted` → throw; no finish → throw, never empty output), and accepts `system` + `reasoningEffort` options. `llmJson` extracts the first `{…}` JSON object from the completion with a string-aware brace scanner (braces inside JSON strings never affect the depth count); if none is found it does **one** bounded retry quoting the offending reply. `resolveCustomAnswer` uses the optional `recommender` config route else the parent model at its lowest declared effort. If the parent cannot map the text, the user is asked **once more** with `“<text>” does not match a configured model. Choose one:`; still unresolved, the spawn proceeds untouched.
- **Per-ask `choiceMap` + idempotent wrapping.** Each ask owns its own `Map<label, choice>`, so concurrent spawns never collide. Wrapping is idempotent via `Symbol.for('spawn-model-choice.wrapped')` and dispose restore only unwraps when the installed methods are still this plugin's wrappers. `hasExplicitModelChoice` checks only `provider`/`model`/`reasoningEffort` — sizing keys like `maxTokens` no longer suppress the chooser — and the skip guard tests **both** `agentOptions` and `modelSelection`.
...
Children inherit a frozen route without explicit `agentOptions` — the parent's model/effort is baked into the delegation call. Without this plugin, every spawn silently reuses the configured default even when the task wants deeper reasoning or a cheaper model. This plugin asks the human per spawn (optional, fail-open) and injects the chosen route via `agentOptions` (`provider`, `model`, `reasoningEffort` — honored on every harness) plus the canonical `modelSelection` field (which harnesses with the upstream support install on the child before its first prompt assembly), before the real `start`/`startContinuable` runs.

## Known limitations

- The free-text resolver (`deterministicMatch` + `effortFromText`) is deliberately conservative — identifier-boundary matching means a substring like `high` inside `xhigh` never matches, and ambiguous text must fall through to the bounded LLM shortlist or the follow-up ask.
- An LLM recommendation (`recommender` configured) requires a live route for that provider/model; without one the classifier call fails and no tier is tagged (deterministic default needs no route at all).
